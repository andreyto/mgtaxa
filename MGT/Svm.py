...
class TaxaSampler(PhyOptions):
    ...
    def subSampleCountsNonViral(self):
        maskVir = self.taxaTree.buildSubtreeMask(ids=(self.viralRootTaxid,), values=(1,))
        kmerCounts = numpy.resize(KmerCounts(self.kmerPathSamp).load(),len(maskVir))
        kmerCountsVir = numpy.resize(KmerCounts(self.kmerPathVir).load(),len(maskVir))
    ...
    def writeViralVsNonViralTrainingSet(self):
        ...
        for (inpPath,label) in zip(inpPaths,labels):
            ...
            cntTaxidVir = numpy.resize(max(len(cntTaxidVir),len(counts)))
    ...
    def buildViralRankLabels(self,rank,indTaxid,testRatio,minKmerRankTrain,maxKmerRankTrain,permutMethod='sort',
        ...
        ##kmerCnt = numpy.resize(self.assignNodeCounts(),len(maskRank))
        ...
        kmerCnt = numpy.resize(kmerCnt,len(maskRank))
